...
export default function Testimonials() {
    ...
    const columns = [
        ...
        { id: 'priorityNo', label: 'Priority No', align: 'center', minWidth: 170 },
        {
            ...
            renderCell: (params) => {
                return (<Avatar sx={{ margin: "auto", cursor: "pointer" }} variant="square"
                    src={`${API_ENDPOINT}/testimonialImages/${params?.image}`}
                    onClick={() => {
                        setOpen(true);
                        setImageName(`${API_ENDPOINT}/testimonialImages/${params?.image}`);
                    }}>
                    <AssignmentIcon />
                </Avatar>)
            }
        ...
        //     renderCell: (params, id) => {
        //         console.log(params);
        //         return <IconButton type="button" onClick={() => {setDialogOpen(true);setParams(params);setId(params?._id)}} aria-label="edit">
        //         <Edit/>
        //     </IconButton> 
        //     }
        // },
    ...
    const getUsers = async () => {
        ...
        const { status, data: { data } } = await axios.get(`${API_ENDPOINT}/api/retrieve/testimonials`, { headers });
        if (status === 200) {
            setSuccess(true);
            setRows(data);
        }
    ...
    const updateUser = async () => {
        try {
            const headers = { Authorization: `Bearer ${token}` };
            const { status, data: { message } } = await axios.post(`${API_ENDPOINT}/api/update/testimonial`, {
                testimonialId: id2,
                isActive: isActive
            }, { headers });
    ...
    const handleDelete = async (Id) => {
        try {
            ...
            const { status } = await axios.patch(
                `http://localhost:4210/api/delete/testimonial/${Id}`,
                null, 
                {
                    headers: headers,
                }
        ...
        <Grid>
            <Grid >
                <Grid container sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <Grid xs={12} md={4}>
                        <Typography variant={"h5"} sx={{ margin: "10px" }}>Testimonials</Typography>
                    </Grid>
                    <Grid xs={12} md={4} sx={{ p: { xs: '10px', md: "0" } }}>
                        ...
                        >
                            <InputBase
                                ...
                                inputProps={{ 'aria-label': 'search testimonial' }}
                            />
                    ...
                    <Grid xs={12} md={4} sx={{ display: "flex", justifyContent: { xs: "start", md: "end" } }} >
                        <Button sx={{ margin: "10px" }} color="success" variant="contained" onClick={() => setDialogOpen(true)}>Add Testimonial</Button>
                    </Grid>
                </Grid>
            ...
            <TestimonialDialog open={dialogOpen} getUsers={getUsers} params={params} id={id} onClose={() => setDialogOpen(false)} />
        </Grid>
    );
}