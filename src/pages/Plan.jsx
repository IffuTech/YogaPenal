...
export default function Plan() {
    ...
    const columns = [
        ...
        { id: 'price', label: 'Price', align: 'center', minWidth: 170 },
    ...
    const updateUser = async () => {
        try {
            const headers = { Authorization: `Bearer ${token}` };
            const { status, data: { message } } = await axios.post(`${API_ENDPOINT}/api/update/plan`, {
                _id: id2,
                isActive: isActive
            }, { headers });
    ...
    const handleDelete = async (planId) => {
        try {
            ...
            const { status } = await axios.patch(
                `http://localhost:4210/api/delete/plan/${planId}`,
                null, 
                {
                    headers: headers,
                }
        ...
        <Grid>
            <Grid >
                <Grid container sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <Grid xs={12} md={4}>
                        <Typography variant={"h5"} sx={{ margin: "10px" }}>Plans</Typography>
                    </Grid>
                    ...
                    <Grid xs={12} md={4} sx={{ display: "flex", justifyContent: { xs: "start", md: "end" } }} >
                        <Button sx={{ margin: "10px" }} color="success" variant="contained" onClick={() => setDialogOpen(true)}>Add Plan</Button>
                    </Grid>
                </Grid>
            ...
            <PlanDialog open={dialogOpen} getBlogs={getBlogs} params={params} id={id} onClose={() => setDialogOpen(false)} />
        </Grid>
    );
}