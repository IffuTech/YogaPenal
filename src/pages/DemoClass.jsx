...
export default function DemoClass() {
        ...
        { id: 'priorityNo', label: 'Priority No', align: 'center', minWidth: 170 },
        {
            ...
            renderCell: (params) => {
                return (<Avatar sx={{ margin: "auto", cursor: "pointer" }} variant="square"
                    src={`${API_ENDPOINT}/demoVideos/${params?.video}`}
                    onClick={() => {
                        setOpen(true);
                        setImageName(`${API_ENDPOINT}/demoVideos/${params?.video}`);
                    }}>
                    <AssignmentIcon />
                </Avatar>)
            },
    ...
    const getUsers = async () => {
        ...
        const { status, data: { data } } = await axios.get(`${API_ENDPOINT}/api/retrieve/demos`, { headers });
        if (status === 200) {
            setSuccess(true);
            setRows(data);
        }
    ...
    const updateUser = async () => {
        try {
            const headers = { Authorization: `Bearer ${token}` };
            const { status, data: { message } } = await axios.post(`${API_ENDPOINT}/api/update/demo`, {
                demoId: id2,
                isActive: isActive
            }, { headers });
    ...
    const handleDelete = async (demoId) => {
        try {
            ...
            const { status } = await axios.patch(
                `http://localhost:4210/api/delete/demo/${demoId}`,
                null, 
                {
                    headers: headers,
                }
            ...
            if (status === 200) {
                enqueueSnackbar(` Demo deleted successfully`, { variant: "success" });
            } else {
        ...
        <Grid>
            <Grid >
                <Grid container sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <Grid xs={12} md={4}>
                        <Typography variant={"h5"} sx={{ margin: "10px" }}>Demo Classes</Typography>
                    </Grid>
                    <Grid xs={12} md={4} sx={{ p: { xs: '10px', md: "0" } }}>
                        ...
                        >
                            <InputBase
                                ...
                                inputProps={{ 'aria-label': 'search demos' }}
                            />
                    ...
                    <Grid xs={12} md={4} sx={{ display: "flex", justifyContent: { xs: "start", md: "end" } }} >
                        <Button sx={{ margin: "10px" }} color="success" variant="contained" onClick={() => setDialogOpen(true)}>Add Demo Class</Button>
                    </Grid>
                </Grid>
            ...
            <ImageDialog open={open && imageName} onClose={() => setOpen(false)} video={imageName} />
            <DemoDialog open={dialogOpen} getUsers={getUsers} params={params} id={id} onClose={() => setDialogOpen(false)} />
        </Grid>
    );
}