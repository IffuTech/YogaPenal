...
export default function TestimonialDialog(props) {
    ...
    if (props?.id) {
        ...
        formData.append('testimonialId', userdata?._id);
    } else {
        formData.append('image', selectedImage);
        formData.append('title', userdata?.title);
        formData.append('type', alignment);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let url;
            if (props?.id) url = `${API_ENDPOINT}/api/update/testimonial`
            else url = `${API_ENDPOINT}/api/create/testimonial`
    ...
    return (
        <div>
            <Dialog open={open} onClose={() => { onClose(); setUserData({}) }}>
                <DialogTitle>{props?.id ? "Update Testimonial" : "Add Testimonial"}</DialogTitle>
